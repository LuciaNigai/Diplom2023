...
$stmt = $con->prepare("select users.name, users.email, users.user_photo,  post_info.post_category, post_info.post_state, post_info.post_description, post_info.post_photo, post_info.post_date, post_info.post_id from users right join post_info using(id) where post_info.post_category='Medicine' order by post_info.post_date");
$stmt->execute();
